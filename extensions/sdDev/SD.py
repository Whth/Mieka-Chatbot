...
from extensions.dynamic_prompt.wild_card_helper import wd_convertor

sys.path.append("../..")
from extensions.baidu_translater.translater import Translater
...
def sd_draw(
    ...
    steps: int = 16,
    size: list = [512, 768],
    use_sampler: str or int = "UniPC",
    config_scale: float = 6.8 + random.random(),
    output_dir="./output",
    use_doll_lora: bool = False,
    safe_mode: bool = True,
    face_restore: bool = False,
    use_body_lora: bool = False,
    use_ero_TI: bool = False,
    use_honey_lora: bool = False,
    use_echi_lora: bool = False,
    use_wd: bool = True,
) -> list[str]:
    """
    ...
    if type(positive_prompt) != str or not positive_prompt.strip():
        positive_prompt = (
            "pink hair:1.2,hair pin:1.2,girl,student uniform:1.4,uniform,uniform,collar:1.3,"
    ...
    if type(negative_prompt) != str or not negative_prompt.strip():
        ...
        negative_prompt += safe_word
    if use_doll_lora:
        doll_lora_min = 0.06
        doll_lora_normal_max = 0.22
        doll_lora_emphasis_max = 0.86
        doll_lora = (
            f"<lora:japaneseDollLikeness_v10:{uniform(doll_lora_min, doll_lora_normal_max)}>"
    ...
    if use_honey_lora:
        ...
        honey_lora_min = 0.65
        honey_lora_list = [
            f"<lora:HinaIAmYoung22_zny10:{uniform(honey_lora_min, honey_lora_max)}>",
    ...
    if use_body_lora:
        ...
        body_lora_min = 0.06
        body_lora = (
            f"<lora:hyperbreasts_v5Lora:{uniform(body_lora_min, body_lora_max)}>, "
...
def sd_diff(
    ...
    steps: int = 15,
    use_sampler: str or int = "UniPC",
    denoising_strength: float = 0.74,
    config_scale: float = 7,
    output_dir: str = "./output",
    fit_original_size: bool = True,
    ...
    if type(use_sampler) == str and use_sampler in samplers_list:
        usedSampler = use_sampler
    elif isinstance(use_sampler, int) and 0 <= use_sampler < len(samplers_list):
        usedSampler = samplers_list[use_sampler]
    else:
        usedSampler = "DPM++ SDE Karras"
...
def npl_reformat(
    ...
    split_keyword: str = None,
    specify_batch_size: bool = False,
