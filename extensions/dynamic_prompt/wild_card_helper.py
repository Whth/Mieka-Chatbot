...
from extensions.dynamic_prompt.config import wd_dir
from extensions.dynamic_prompt.config import wd_presets


def extract_wild_card_fname(
    root_path: str = "./",
    suffix: str = ".txt",
...
def wd_convertor(
    string: str, wd_root: str = wd_dir, sign: str = "__", bad_wd_remove: bool = True
) -> str:
    """
    ...
    wd_dict: dict = extract_wild_card_fname(
        wd_root, make_card_path_dict=True, grammar_style=True
    )
...
def random_return_card_content(
    card_path: str, strength: float = 1.0, append_comma: bool = False
) -> str:
    """
    :param append_comma:
    :param card_path:
...
def make_wd_preset(card_dirs: list[str], save_dir: str):
    wd_dict = {}
    for wd in card_dirs:
        wd_dict[re.split(pattern=r"\\|/", string=wd)[-1]] = extract_wild_card_fname(
            wd, grammar_style=True
        )
