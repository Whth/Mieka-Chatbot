...
from extensions.sdDev import sd_draw, sd_diff, deAssembly, npl_reformat
    ...
    with open("Bots.json", mode="r") as f:
        ...
        app = Ariadne(
            config(
                ...
                account=botInfo.get("account")[0],  # 你的机器人的 qq 号
            ),
        )
...
def message_constructor(
    string: str or list[str], img: str or list[str], random_string: bool = True
) -> MessageChain:
    """
    ...
    elif (isinstance(string, list) and random_string is False) or isinstance(
        string, str
    ):
...
async def groupDiffusion(
    ...
    if Image in chain:
        ...
        await app.send_message(
            group, message_constructor(finishResponseList, generated_path)
        )
    else:
        print("using txt2img")
        ...
        if random.random() < REWARD_RATE and use_reward:
            ...
            await app.send_message(
                group, message_constructor(rewardResponseList, generated_path_reward)
            )


@app.broadcast.receiver("GroupMessage", decorators=[MentionMe()])
async def img_gen_group_atme(
    app: Ariadne, member: Member, group: Group, chain: MessageChain = MentionMe(False)
):
    """
    快速响应群at
    :param app:
    ...
    await group_fast_feed(app, group, chain)
...
async def img_gen_group_quiet(
    app: Ariadne, member: Member, group: Group, chain: MessageChain
):
    """
    ...
    if "+" not in str(chain):
        return
    pos_prompts, neg_prompts, batch_size = deAssembly(
        str(chain), specify_batch_size=True
    )
    if batch_size > 7:
        batch_size = 7
    await groupDiffusion(
...
async def img_gen_friend(app: Ariadne, friend: Friend, chain: MessageChain):
    ...
    if "mika" in str(chain):
        pos_prompts, batch_size = npl_reformat(
            str(chain), split_keyword=dear_name, specify_batch_size=True
        )
    else:
        pos_prompts, neg_prompts, batch_size = deAssembly(
            str(chain), specify_batch_size=True
    ...
    if Image in chain:
        ...
        await app.send_message(
            friend, message_constructor(finishResponseList, generated_path)
        )
    else:
        print("using txt2img")
        ...
        for _ in range(batch_size):
            while True:
                size = [
                    random.randint(MIN_PIXEL, MAX_PIXEL),
                    random.randint(MIN_PIXEL, MAX_PIXEL),
                ]
        ...
        if random.random() < REWARD_RATE and use_reward:
            ...
            await app.send_message(
                friend, message_constructor(rewardResponseList, generated_path_reward)
            )
...
async def good_morning(channel: Ariadne = app):
    ...
    prompt = (
        "pink hair girl:1.4,blue eyes,student uniform:1.3,standing:1.3,landscape:1.5,blue sky,morning,"
    ...
    await channel.send_group_message(
        groups_list[0], message_constructor(goodMorning, generated_path)
    )
...
async def live(channel: Ariadne = app):
    ...
    if scheduler_config.get("live_enabled"):
        ...
        for _ in range(live_times):
            ...
            prompt = (
                "large long breasts,sexy,headdress,1girl,"
                f"{additional_prompt},"
            ...
            for master_account in scheduler_config.get("masters"):
                await channel.send_friend_message(
                    master_account, message_constructor(masterList, generated_path)
                )
...
async def echi(channel: Ariadne = app):
    ...
    if scheduler_config.get("echi_enabled"):
        ...
        for _ in range(live_times):
            ...
            prompt = "1girl:1.2,solo" f"{additional_prompt},"
            ...
            for master_account in scheduler_config.get("masters"):
                await channel.send_friend_message(
                    master_account, message_constructor(eroList, generated_path)
                )
            time.sleep(live_interval * 60)
...
async def check_key(chain: MessageChain):
    ...
    for keyword in keywords:
        ...
        if enabled is None:
            enabled = (
                False if re.match(f"#{keyword} {end_keyword}", str(chain)) else None
            )
